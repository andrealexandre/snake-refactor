...
import snake.settings.SnakeSpeed
...
class OptionsFrame(caller: GameMenu) : JFrame("Options"), ActionListener {
    ...
    private fun createGameConfiguration() {
        caller.config = GameConfiguration(
            snakeSpeed = when {
                snakeSpeedEasy.isSelected -> SnakeSpeed.EASY
                snakeSpeedMedium.isSelected -> SnakeSpeed.MEDIUM
                snakeSpeedHard.isSelected -> SnakeSpeed.HARD
                else -> SnakeSpeed.MEDIUM
            },
            labyrinthPath = lp.getLabyrinthName()
        )
    }
    ...
    private inner class LabyrinthPreviewer : JPanel(), ActionListener {
        ...
        fun getLabyrinthName(): String? {
            return if (idx < 0 || idx >= labyrinths.size) null else labyrinths[idx]
        }
